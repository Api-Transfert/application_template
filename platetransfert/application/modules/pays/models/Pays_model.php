...
class Pays_model extends CI_Model 
{
	...
	public function get_all_pays($id="")
	{
		$this->db->select('paysName,paysCode,paysCode3,paysDestination,currencyCode,currencyName')
				 ->from('pays')
				 //->join('pays','pays.paysId = structure.structurePaysId')
				 ->join('currency', 'currency.currencyId=pays.paysCurrencyId');
				// ->join('permissions', "permissions.perm_id = group_perm.perm_id")
				 //->where('pays.paysCurrencyId','currency.currencyId');
		$query = $this->db->get();
		return $query->result(); 
	}
